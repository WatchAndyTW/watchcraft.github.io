...
copyright(){
    output "翼龍面板安裝 & 升級程序 v3"
    output "想要免費的1G服務器嗎? BlueHosting! https://discord.blueserv.ga/"
}

get_distribution(){
    output "感謝您的支持. 請注意這個程序只能在全新的運作系統運行. 如果在較舊的運作系統運行可能會無法運行."
    output "自动操作系统检测已初始化."
    ...
    if [ "$lsb_dist" =  "ubuntu" ]; then
        if [ "$dist_version" != "18.10" ] && [ "$dist_version" != "18.04" ] && [ "$dist_version" != "16.04" ] && [ "$dist_version" != "14.04" ]; then
            output "不支持的Ubuntu版本。 仅支持Ubuntu 18.04,16.04和14.04."
            exit 2
        fi
	elif [ "$lsb_dist" = "debian" ]; then
        if [ "$dist_version" != "9" ] && [ "$dist_version" != "8" ]; then
            output "不支持的Debian版本。 仅支持Debian 9和8."
            exit 2
		fi
    elif [ "$lsb_dist" = "fedora" ]; then
        if [ "$dist_version" != "29" ] && [ "$dist_version" != "28" ]; then
            output "不支持的Fedora版本。 仅支持Fedora 29和28."
            exit 2
        fi
    elif [ "$lsb_dist" = "centos" ]; then
        if [ "$dist_version" != "7" ]; then
            output "不支持的CentOS版本。 仅支持CentOS 7."
            exit 2
        fi
...
get_virtualization(){
    ...
    if [ "$virt_serv" != "" ] && [ "$virt_serv" != "kvm" ] && [ "$virt_serv" != "vmware" ] && [ "$virt_serv" != "hyperv" ] && [ "$virt_serv" != "openvz lxc" ]; then
        warn "不支持的虚拟化方法. 有关您的服务器是否可以运行Docker，请咨询您的提供商. 继续需要您承擔担风险."
        warn "如果您的服务器在将来的任何时候中断，将不会给予支持."
        warn "接受?\n[1] Yes.\n[2] No."
        read choice
        case $choice in 
    ...
    if echo $(uname -r) | grep -q xxxx; then
        output "OVH Kernel 已偵測. The script will not work. Please install your server with a generic/distribution kernel."
        exit 6
    elif echo $(uname -r) | grep -q pve; then
        output "Proxmox LXE Kernel 已偵測. 您已选择继续执行最后一步，因此您將自行承担风险."
        output "继续进行危险的安裝 請放心我會努力的..."
    elif echo $(uname -r) | grep -q stab; then
        if echo $(uname -r) | grep -q 2.6; then 
            output "OpenVZ 6 已偵測. 該服務器都無法使用Docker. 這個程序將退出以避免進一步的損害."
            exit 6
        fi
    elif echo $(uname -r) | grep -q lve; then
        output "CloudLinux Kernel 已偵測. 該服務器都無法使用Docker. 這個程序將退出以避免進一步的損害."
        exit 6
...
server_options() {
    ...
    output "[2] 只安裝主機."
    output "[3] 安裝面板和主機."
    output "[4] 安裝 SFTP 服務器 (必須安裝主機后才能使用. Ubuntu 14.04 不支持使用此安裝.)"
    output "[5] 升級 0.7.x 面板到 0.7.13."
    output "[6] 升級 0.6.x 主機到 0.6.12."
    output "[7] 升級SFTP至 1.0.4."
    output "[8] 安裝或者升級 phpMyAdmin 4.8.5 (必須安裝面板后才能使用.)"
    output "[9] 切換一個翼龍面板主題."
    output "[10] 緊急切換 MariaDB root 密碼."
    output "[11] 緊急重置數據庫架設咨詢."
    warn "[12] 启用外部Mariadb root登录.（警告：这是非常不鼓励的，因为它存在安全风险）"
    output "[13] 禁止外部Mariadb root登录. (Reversing [12].)"
    read choice
    case $choice in
        1 ) installoption=1
            output "您選擇了只安裝面板."
...
dns_check(){
    ...
    if [ "${SERVER_IP}" != "${DOMAIN_RECORD}" ]; then
        output "您輸入的網域并沒有連接此主機的IP地址."
        output "請使用 A 記錄 連接您的IP地址到網域. 例子, 如果您成功添加了一個Ａ記錄　他的名字是 "Panel", 那麽您的 FQDN 就是 panel.yourdomain.tld"
